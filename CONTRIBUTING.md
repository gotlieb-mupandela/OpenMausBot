...
Data lives in `~/.aishe/` (bots, transcripts, per-thread NDJSON event logs, config with keys).
...
- **Never touch the real `~/.aishe`.** The setup file points `HOME` at a temp dir; keep it
  that way.
...
API keys are write-only: they land in `~/.aishe/config.json` via `PUT /api/config` and the API
