...
const DENY_TIMEOUT_NOTE =
  "Aishe: nobody answered this permission request in time. Skip this action and finish what you can without it.";
const QUESTION_TIMEOUT_NOTE = "Aishe: nobody answered in time. Use your best judgment and continue.";
  ...
  return {
    ...
    close() {
      for (const p of [...pending.values()]) {
        if (p.ask.kind === "question") p.finish("answer", "Aishe: the turn is ending — wrap up.", "shutdown");
        else p.finish("deny", "Aishe: the turn ended", "shutdown");
      }
      try {
        server.close();
      } catch {}
