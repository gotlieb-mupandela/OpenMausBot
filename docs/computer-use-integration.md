# Computer use & browser use in Aishe

Decision doc, 2026-08-12. How bots in Aishe get local computer use and
...
│     one TCC prompt, named Aishe          │ unix socket (private)
...
1. **Spawn from the Electron main process, never from the server/gateway
   ...
   Spawned from Electron main → the grant is Aishe's, users see ONE
   prompt named Aishe, and the bundled driver inherits it. Spawned from
...
- Ship the binary at `Aishe.app/Contents/Resources/cua-driver`,
...
{ "mcpServers": { "computer": {
    ...
    "env": { "CUA_DRIVER_EMBEDDED": "1", "CUA_DRIVER_HOST_BUNDLE_ID": "com.aishe.app" }
} } }
```
